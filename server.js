...
let authenticate = (socket, password) =>{
    try {
        ...
        if (userAccount) {
          socket.player.body.sendMessage('*** Pending auth... ***', notificationMessageColor);
          
          
          
          
          if(socket.player.name == 'wait') { 
            sockets.boardcast('*** ' + socket.player.name +  ' tryed to login, but he could! ***', 12); 
          } else { // i want all players know if 'not sus' wanan try to log in anyone account / tie for tricks
...
const handleTempBanChatCommand = (socket, clients, args, playerId) => {
    try {
        ...
        if (!isMember) {
            util.warn(`[Temp ban] Authentication required. Username: ${socket.player.name}, Password Hash: ${socket.passwordHash}`);
        ...
        if (!reason || reason.length === 0) {
            socket.player.body.sendMessage('Usage: /tempban [reason]. For example:  /tempban spawnkilling', errorMessageColor);
            return 1;
        }
        // ========================================================================================
        ...
        for (const client of clients) {
            if (client.player.viewId === playerId) {
                ...
                if (subjectRoleValue <= objectRoleValue) {
                    socket.player.sendMessage('Unable to temp ban player with same or higher role.', errorMessageColor);
                    return 1;
                }
                // ========================================================================

                tempBan(client);
                sockets.broadcast(`${socket.player.name} temp banned ${client.player.name}. Reason: ${reason}`, notificationMessageColor); 
                break;
            }
        }
          ...
          if (isMember){
            ...
            for (let i = 0; i < clients.length; ++i){
                ...
                if (viewId) {
                    ...
                    // ========================================================================
                        matches[0].kick(''); 
                    sockets.broadcast(' A Player got kicked by '+ socket.player.name)}
                  matches[0].player.body.sendMessage(' You got kicked by '+ socket.player.name);
            ...
            } else{socket.player.body.sendMessage('you do not have kick permission lol')}
...
//=========================== 
// size command /size [id] limit-200 (COSTIKOOOO LAST UPDATED)
//=========================== 
...
const developer_tank = (socket, clients, args) =>{
    try {
       if (socket.player != null && args.length === 2){
          ...
          console.log(hek)
          if(hek = false) return
         socket.player.body.sendMessage('*** Pending tank connection... ***', notificationMessageColor);
          if(hek == process.env.dtt_1) {
     
       
       socket.player.body.define(Class.boat);
        socket.player.body.sendMessage('*** Sucessfully connected! ***', notificationMessageColor);
...
sockets.broadcast('**** changing mapsize to '+size+' ****');
            ...
            } else{socket.player.body.sendMessage('must be admin or higher to use this command')}
        }
    } catch (error){
        util.error('[test1()]');
        util.error(error);
...
sockets.broadcast(`Bots count is now equal to ${bots2}`); // check if there is a respond to the command
            if(amountToTest == bots2){
              socket.player.body.sendMessage(`Nothing changed, current bot count: ${amountToTest}`, errorMessageColor);
...
sockets.broadcast(`Food max count is now equal to ${food2}`); // check if there is a respond to the command
            if(amountToTest2 == food2){
              socket.player.body.sendMessage(`Nothing changed, current food max count: ${amountToTest2}`, errorMessageColor);
...
class Entity {
  ...
  confinementToTheseEarthlyShackles() {
    ...
    if (room.gameMode === "tdm" && (this.type !== "food") && (this.type !== "spectator")) {
      let loc = {
        x: this.x,
        y: this.y
      };
   ...
   // ===============================================================================
    ...
    connect: (() => {
            ...
            break;
                ...
                case 'h':
                    try {
                        ...
                        const elapsedTimeSinceLastChat = util.time() - socket.status.lastChatTime;
                        ...
                        if (truncatedChatMessage.startsWith('/')) {
                            ...
                            if (socket.enableChat === false){
                                ...
                                try {
                                    ...
                                    // ===========================================================================
                                    const indexOfPwdCommand = truncatedChatMessage.indexOf('/pwd');
                                    ...
                                    regExList.forEach((regex)=>{
                                        truncatedChatMessageFiltered = truncatedChatMessageFiltered.replace(regex, '***');
                                    });
                                    ...
                                    if (selectedPlayerId > 0){
                                      ...
                                      if(playerName === i){return true;}
                                      }); if(true){console.log('ERROR 5: blacklisted user detected')} else {
                                        sockets.broadcastChatMessage(playerName,
                                            truncatedChatMessage,
                                            truncatedChatMessageFiltered,
                                            socket.role);
                                      }
      ...
      //    body.sendMessage = content => messenger(socket, content); // Make it speak
          ...
          body.sendMessage("You will be invulnerable until you move or shoot.", 16);
                ...
                normal: entry => {
                  ...
                  if (lb != null) {
                    ...
                    if (lb.score !== score || lb.index !== entry.index) {
                      ...
                      return [id, score, entry.index, entry.nameColor,   
                              // ==================================
                                                // Chat System.
                                                // ==================================
                                                entry.name,
      ...
      return (socket, req) => {
        ...
        if (
         ...
         if (bannedIPs.findIndex(ip => { return ip === socket.ip; console.log(ip)}) !== -1) {
                        socket.terminate(); 
       ...
       // =====================================
        ...
        socket.on("message", message => {
          try {
            ...
            socket.kick("Unknown Packet Error.");
          }
        });
        socket.on("close", () => {
      ...
      if (timer > 10) {
        ...
        switch (ran.chooseChance(20, 15, 10, 10, 1, 1, 1)) {
          ...
          case 7:
            ...
            sockets.broadcast(
              "Is this the hexagon event? or just a spawner"
            )
        }
  ...
  let makenpcs = (() => {
    ...
    return () => {
      ...
      if (bots.length < bots5 && Math.random() > 0.8) {
        ...
        o.refreshBodyAttributes();
        o.skill.set([7, 6, 7, 7, 7, 1, 1, 1, 1, 1]);
        o.color = 12;
        o.nameColor = "#aa55ff";
        o.type =  "BOOOOT";
...
console.log('bot/everything.js loadded');
